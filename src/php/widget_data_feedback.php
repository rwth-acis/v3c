...
$stmt = $conn->prepare("SELECT widget_data_feedback_lng.element_id, title, `text`, lang
                        FROM widget_data_feedback_lng, course_elements
                        WHERE widget_data_feedback_lng.element_id = course_elements.id
...
if ($stmt->execute() !== FALSE) {
    ...
    echo '{"element_id": '. $res['element_id'] .', "title": "'. $res['title'] .'", "text": "'. $res['text'] .'"}';
}
else {
  http_response_code(404);
  echo "Not found";
